...
""")
    ...
    def loader(self):
        ...
        async def mcf(self:MCFSignup, ctx):
            self.tournament_data = await database.McfDataUtils(ctx, mcf_database).get_latest_tournament()

            if not self.tournament_data == None:
                # Continue with command.
                return True
            
            else:
                # Notify member that there is no mcf yet.
        ...
        async def join(self:MCFSignup, ctx):
            tournament = database.Tournament(ctx, self.tournament_data)
            ...
            if tournament.is_form_open(data):
                    ...
                    await send_modal(ctx, 
                        GoldyBot.utility.views.forms.normal_form(
                            ...
                            items = [
                                ...
                                GoldyBot.nextcord.ui.TextInput(
                                    label=f"Will you make it? ({self.tournament_data.time_and_date.strftime('%A, %d %b')})", 
                ...
                # Notify member that the form is closed.
